...
Create FAQ post in `_faq` folder (categories are optional):
```yml
---
...
{% raw %}
```yaml
{% include faq.html multiple="true" title="Frequently asked questions" category="presale" subtitle="Find quicke answers to frequent pre-sale questions asked by customers" %}
```
{% endraw %}
